...
-- Improved snow effect for black screen
local snowParticles = {}
...
for i = 1, 100 do
    ...
    snow.Position = UDim2.new(0, math.random(0, 1000), 0, math.random(-100, 0))
    snow.ZIndex = 10
    snow.Parent = snowContainer
    
    local speed = math.random(50, 150)
    table.insert(snowParticles, {particle = snow, speed = speed})
end

game:GetService("RunService").RenderStepped:Connect(function(delta)
    for _, snowData in pairs(snowParticles) do
        local snow = snowData.particle
        local speed = snowData.speed
        
        snow.Position = snow.Position + UDim2.new(0, 0, 0, speed * delta)
        
        if snow.Position.Y.Offset > 650 then
            snow.Position = UDim2.new(0, math.random(0, 1000), 0, -10)
        end
    end
end)
